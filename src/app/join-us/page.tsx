  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({
        type: "success",
        message:
          "Thanks for registering your interest. We will contact you soon.",
      });
    } catch (error) {
      const message =
    ...
    <main className="w-full bg-white font-sans text-slate-800">
      ...
      <section className="container mx-auto px-4 md:px-8 lg:px-16 py-10 md:py-14 text-center">
        ...
        <p className="text-xs md:text-sm text-slate-600 leading-relaxed max-w-3xl mx-auto">
          Evolve Sangh Foundation is built on the belief that change happens
          when communities come together. Whether you choose to volunteer your
          time, share your skills, partner as a corporation, or donate — you
          become a vital part of our movement to uplift lives across India.
        </p>
      </section>
        ...
        <div className="container mx-auto px-4 md:px-8 lg:px-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {waysToJoin.map((item) => (
              <div
                key={item.id}
                className="bg-white border border-gray-200 rounded-sm p-6 flex items-start gap-5 shadow-sm hover:shadow-md transition-shadow"
              >
                <div className="text-[#005089] shrink-0">{item.icon}</div>
                <div>
