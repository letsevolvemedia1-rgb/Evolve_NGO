...
const campaigns = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      <>
        To address this urgent challenge, Evolve Sangh Foundation&apos;s{" "}
        <strong>&apos;Hunger Free Night&apos;</strong> campaign is ensuring that
        no one sleeps on an empty stomach. Through free food distribution drives
        in slums, support for underprivileged school children, and provision of
        meals to struggling youth and their families, we are bringing hope,
        health, and dignity to the most vulnerable.
      </>,
      <>
        But this mission is far bigger than us.{" "}
        <strong>We cannot do it alone.</strong>
        <br />
        Your support can change this.
        <br />
        By contributing to Hunger Free Night, you will: Ensure children sleep
        with a full stomach. Create a ripple of compassion that uplifts entire
        communities.
      </>,
      <>
...
function SupportACauseContent() {
  ...
  const initialIndex =
    causeParam === "food" ? 1 : causeParam === "future" ? 2 : 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.error || "Unable to submit the donation form right now.",
        );
      }
      ...
      if (!razorpayLoaded || !window.Razorpay) {
        throw new Error(
          "Unable to load Razorpay checkout. Please check your internet and try again.",
        );
      }
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unable to submit the donation form right now.";
            ...
            <div className="border border-gray-200 rounded-sm shadow-sm transition-shadow hover:shadow-md duration-300 bg-white">
              ...
              <form onSubmit={handleSubmit} className="px-6 py-6 space-y-5">
                ...
                <p className="text-[10px] md:text-xs text-center text-slate-500 uppercase tracking-wide font-medium">
                  YOUR DONATION WILL HELP UPLIFT THE FUTURE OF NEEDY CHILDREN
                  AND FAMILIES
                </p>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <input
                      ...
                      className="border border-gray-300 rounded-sm px-3 py-2 text-xs md:text-sm bg-gray-50 text-slate-500"
                    />
                  </div>
                ...
                <div className="bg-gray-50 px-4 py-3 rounded-sm text-center">
                  <p className="text-[9px] md:text-[10px] text-slate-500 leading-relaxed uppercase">
                    *YOUR CONTRIBUTIONS ARE ELIGIBLE FOR UPTO 50% TAX BENEFIT
                    UNDER SECTION 80G AS EVOLVE SANGH FOUNDATION IS REGISTERED
                ...
                <label className="flex items-start gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={handleInputChange}
                    className="mt-0.5 w-4 h-4 accent-[#005089]"
                  />
                  <span className="text-[9px] md:text-[10px] text-slate-500 leading-relaxed">
                    ...
                    through whatsapp/email/phone to provide information of your
                ...
                <div className="text-center">
                  <button
                    ...
                    className="bg-[#8cc63f] hover:bg-[#7cb036] text-white font-bold uppercase text-sm px-8 py-3 rounded-sm transition-colors cursor-pointer tracking-wider disabled:cursor-not-allowed disabled:opacity-70"
                  >
                    {isSubmitting
                      ? "PROCESSING..."
                ...
                {status.type !== "idle" && (
                  <p
                    className={`text-sm text-center ${status.type === "success" ? "text-green-700" : "text-red-600"}`}
                  >
                    {status.message}
                  </p>
                )}
...
export default function SupportACausePage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen w-full flex items-center justify-center text-[#005089] font-bold">
          Loading campaign...
        </div>
      }
    >
